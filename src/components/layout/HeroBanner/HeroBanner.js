  ...
  return `
    <section class="hero-banner" style="background-image: url('${thumb}')" aria-label="Destaque">
      ...
      <div class="hero-content container">
        ...
        <div class="hero-actions flex items-center gap-3 md:gap-5 flex-wrap">
          <button class="btn btn-primary flex items-center gap-2 px-4 md:px-6 py-2.5 md:py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-md transition-all duration-300 shadow-lg hover:shadow-xl" id="heroPlayBtn" data-video-id="${videoId}" aria-label="Reproduzir">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5"><path d="M8 5v14l11-7z"/></svg>
            <span class="text-sm md:text-base">Assistir</span>
          </button>
          <button class="btn btn-list flex items-center gap-2 px-3 md:px-5 py-2.5 md:py-3 bg-gray-500/50 hover:bg-gray-500/70 text-white font-semibold rounded-md transition-all duration-300 backdrop-blur-sm border border-white/20 hover:border-white/30" id="heroListBtn" data-video-id="${videoId}" aria-label="Minha Lista">
            <svg id="heroListIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 flex-shrink-0">
              <path d="M5 12h14M12 5v14"/>
            </svg>
            <span id="heroListText" class="text-sm md:text-base whitespace-nowrap">Minha Lista</span>
          </button>
          <button class="btn btn-secondary flex items-center gap-2 px-3 md:px-5 py-2.5 md:py-3 bg-white/20 hover:bg-white/30 text-white font-semibold rounded-md transition-all duration-300 backdrop-blur-sm border border-white/20 hover:border-white/30" id="heroInfoBtn" aria-label="Mais informações">
            ...
            <span class="text-sm md:text-base whitespace-nowrap">Mais info</span>
          </button>
        </div>
      </div>
    </section>
...
function updateHeroListButton(isInList) {
  ...
  if (isInList) {
    // Estado: Item na lista (verde)
    listBtn.className = 'btn btn-list flex items-center gap-2 px-3 md:px-5 py-2.5 md:py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-md transition-all duration-300 backdrop-blur-sm border border-green-500/30 hover:border-green-400/50 shadow-lg hover:shadow-xl';
    ...
    // Estado: Item não está na lista (cinza translúcido)
    listBtn.className = 'btn btn-list flex items-center gap-2 px-3 md:px-5 py-2.5 md:py-3 bg-gray-500/50 hover:bg-gray-500/70 text-white font-semibold rounded-md transition-all duration-300 backdrop-blur-sm border border-white/20 hover:border-white/30';
